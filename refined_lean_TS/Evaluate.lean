        ...
        | (st1, .Rval (env1, decs)) =>
          if st1.clock == 0 then
            ...
            | (st2, .Rval env2) =>
              ...
              | some (x, es2) =>
                ({ st2 with eval_state_field := reset_env_generation st'.eval_state_field es2 }, .Rval [x])
              | none => (st2, .Rerr (.Rabort .Rtype_error))
            | (st2, .Rerr (.Rabort a)) => (st2, .Rerr (.Rabort a))
            | (st2, .Rerr e) =>
              ({ st2 with eval_state_field := reset_env_generation st'.eval_state_field st2.eval_state_field }, .Rerr e)
